...
namespace GentrysQuest.Game.Screens
{
    public partial class CombatTutorial : GqScreen
    {
        ...
        {
            enemy.AiState = AiState.FollowGoal;
            enemy.FocusedPosition = enemy.Position + MathBase.GetDirection(enemy.Position, player.Position) * 500;
            enemy.GetBase().AddEffect(new Swiftness((int)dialogueEvent.Duration + 1000, 15));
            Scheduler.AddDelayed(() =>
            {
                enemy.AiState = AiState.HardPursue;
            }, dialogueEvent.Duration + 1000);
